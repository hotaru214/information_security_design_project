...
把 Windows/Linux 主机日志解析成全组统一的标准安全事件JSON（契约见 `../docs/数据格式契约-v1.md`）。
...
| Windows Security .evtx | 4625 登录失败 | login_failure（含SubStatus失败原因） |
| Windows Security .evtx | 4688 进程创建 | process_create（cmdline取决于审核策略，没有就null） |
| Sysmon .evtx | ID 1 进程创建 | process_create（cmdline+父进程+哈希） |
| Sysmon .evtx | ID 3 网络连接 | network_connect（src/dst IP+端口+协议，对齐网络事件字段） |
| Sysmon .evtx | ID 11 文件创建 | file_create |
| Sysmon .evtx | ID 13 注册表键值 | registry_set |

解析器自动识别日志类型（Security还是Sysmon）。其他事件ID计入统计，不中断。
Day 2 待扩展：4634/4647会话重建、1102、4720、Linux auth.log/auditd、异常预标记规则。
