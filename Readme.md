...
> **输出已对齐全组冻结的 Event V2 契约（2026-09-07）**
...
CLI 运行时会做 **Event V2 契约自检**（19 字段、severity 取值、时间格式等），不合规会直接打印问题。
...
| Zeek 日志目录 | `network_zeek` | `conn.log`（必需）+ `dns.log` / `http.log`（可选），TSV/JSON 均支持，支持 `.gz`；**Zeek uid 会作为 `event_id`，原始日志行进 `raw_log`** |
| CSV 连接日志 | `network_zeek` | 列：`timestamp,src_ip,src_port,dst_ip,dst_port,protocol[,bytes,duration,packets,event_id]`（E 靶场没有 Zeek 时的兜底格式） |

## 3. 检测规则（阈值见 `config.py`，均可用 `--config` 覆盖）

| 规则 | event_type / anomaly_flags[0] | 默认阈值 | ATT&CK（detail.mitre_technique） |
| --- | --- | --- | --- |
...
| 内网横向连接 | `lateral_movement` | 内→内访问 22/23/139/445/3389/5985/5986 | T1021（横向移动） |
...
普通会话事件 event_type：`network_connection` / `dns_query` / `http_request` / `icmp_traffic`。
...
- **event_id**：Zeek 输入用原始 `uid`；PCAP 无原始编号 → `null`（后端不会用内部 id 覆盖）。
...
- **detail**（必填对象）：网络事件特有字段全部在此——`src_port`、`peer_host`、`direction`
  ...
  `tcp_flags`、`dns_queries`、`http_requests`、`icmp_*`；告警事件额外带
  `attack_stage` / `attack_stage_zh` / `mitre_technique` 及各规则证据字段。
...
- 契约校验函数：`normalize.validate_events(events)` 返回问题列表（空=合规），可在后端导入前调用。
...
- ATT&CK 阶段与技术：`detail.attack_stage` / `detail.mitre_technique`（原 `source_type`→`source`、
  `attack_stage` 已移入 detail）。
- 攻击图构建：`src_ip/dst_ip/host/detail.direction`；`lateral_movement` 告警即内网跳板候选链路。
- `anomaly_flags` 可做快速过滤（含规则名与 ATT&CK 技术号）。
...
覆盖：熵计算、内网判定、8 个检测器（正/负用例）、PCAP 端到端、Zeek TSV（uid/raw_log）、
CSV、Event V2 契约（19 字段、null 语义、severity 数字、anomaly_flags、UTC+8 时间）。
...
- 契约变更需全组同步：Event V2 冻结后本模块不再单方面修改 19 个公共字段。
